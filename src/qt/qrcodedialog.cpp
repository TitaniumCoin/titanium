...
{
    QString ret = QString("titanium:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TTN unit input in lnReqAmount, we generate the URI with TTN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(TitaniumUnits::format(TitaniumUnits::TTN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
