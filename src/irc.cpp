...
bool RecvLineFixed(SOCKET hSocket, string& strLine)
{
    ...
    loop
    {
		...
		// +titanium added for irc exits, without this, recv(hSocket, &c, 1, 0); will hang when closing the program after using irc on windows.
		bool stop = false;
		u_long RecvCount;
        while(!stop)
        {
...
void ThreadIRCSeed()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #T1TAN10MTEST3\r");
            Send(hSocket, "WHO #T1TAN10MTEST3\r");
        } else {
            // randomly join #T1TAN10M00-#T1TAN10M99
            int channel_number = GetRandInt(100);
			channel_number = 1; // For now, just use one channel
            Send(hSocket, strprintf("JOIN #T1TAN10M%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #T1TAN10M%02d\r", channel_number).c_str());
        }
